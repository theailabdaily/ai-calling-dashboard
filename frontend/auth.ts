 ...
 *
 *   - Provider: Google OAuth (any Google account — domain restriction removed)
 *   - Audit logging: every signin / signout is POSTed to the backend
 *     /internal/auth-events endpoint for the admin activity page
 *
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async signIn({ user }) {
      ...
      await logAuthEvent({ email: email || '(no email)', event: 'signin_success' });
      return true;
    },
