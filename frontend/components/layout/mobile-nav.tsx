...
export default function MobileNav() {
  ...
  return (
    <>
      ...
      {/* Drawer — slides in from the left. Always rendered (so the slide-out
          animation works), but translated off-screen when closed.            */}
      <aside
        className={cn(
          'lg:hidden fixed top-0 left-0 bottom-0 w-72 max-w-[85vw] bg-black text-white z-50 flex flex-col transition-transform duration-200',
      ...
      >
        <div className="p-5 border-b border-white/10 flex items-start justify-between">
          ...
          <button
            onClick={() => setOpen(false)}
            className="p-2 -mr-2 -mt-1 rounded-lg hover:bg-white/10 active:bg-white/20 transition-colors"
